...
# Injection CSS pour forcer la police Times New Roman
st.markdown(
    """
    <style>
    html, body, [class*="css"] {
...
capacite_monture = st.sidebar.number_input("Capacite monture (kg) - Ex: Boeuf", value=1620)
        ...
        with st.spinner('Extraction des donnees en cours...'):
            try:
                ...
                for key, achat_data in prix_villes.items():
                    ...
                    if not vente_mn or volume < min_volume:
                        continue
                    ...
                    profit_trajet = profit * qte_max
                    score_liquidite = profit * volume
                    ...
                    lignes.append({
                        ...
                        "Vente (MN)": vente_mn,
                        "Profit Net": profit,
                        "Profit TRAJET": profit_trajet,
                        "Score Liquidite": score_liquidite,
                        "Vol/J": volume,
                        "Stabilite %": stabilite,
                ...
                if not lignes:
                    ...
                    df_final = df_final.sort_values("Profit TRAJET", ascending=False).reset_index(drop=True)
                    df_final.index += 1

                    st.success(f"{len(df_final)} opportunites trouvees.")
                    
                    df_style = df_final.copy()
                    df_style['Achat'] = df_style['Achat'].map('{:,.0f}'.format)
                    df_style['Vente (MN)'] = df_style['Vente (MN)'].map('{:,.0f}'.format)
                    df_style['Profit Net'] = df_style['Profit Net'].map('{:,.0f}'.format)
                    df_style['Profit TRAJET'] = df_style['Profit TRAJET'].map('{:,.0f}'.format)
                    df_style['Score Liquidite'] = df_style['Score Liquidite'].map('{:,.0f}'.format)
                    ...
                    st.write(f"Achat : {top['Ville']} pour {top['Achat']:,} | Vente MN : {top['Vente (MN)']:,}")
                    st.write(f"Profit Boeuf : {top['Profit TRAJET']:,} Silver")
                    
                    if webhook_discord:
                        if st.button("Alerter sur Discord (Arbitrage)", type="primary"):
                            msg = {
                                "content": f"[ALERTE ARBITRAGE]\nObjet : {top['Objet']} (Enchant {top['Enchant']}, {top['Qualite']})\nTrajet : {top['Ville']} -> Caerleon\nProfit Total Estime : {top['Profit TRAJET']:,} Silver\n(Volume : {top['Vol/J']}/jour)"
                            }
...
# ==========================================
# ONGLET 2 : FORGE ROYALE (SIGILS INTER-VILLES)
# ==========================================
with tab2:
    ...
    # Generation de toutes les combinaisons avec enchantements
    ...
    def fetch_data_tab2():
        ...
        for i in range(0, len(ITEMS_TAB2), chunk_size):
            ...
            try:
                res_villes = requests.get(url_villes, timeout=15)
                res_mn = requests.get(url_mn, timeout=15)
                if res_villes.status_code == 200: all_villes.extend(res_villes.json())
                if res_mn.status_code == 200: all_mn.extend(res_mn.json())
            except Exception:
                pass
    ...
    def traduire_nom_piece(item_id):
        nom = item_id.split('@')[0] # Retire l'enchantement pour nettoyer le texte
    ...
    if st.button("Lancer l'Analyse Forge Optimisee", use_container_width=True):
        with st.spinner('Extraction des donnees de forge en cours (cette operation peut prendre quelques secondes)...'):
            try:
                ...
                if not df_villes.empty and not df_mn.empty:
                    ...
                    if not df_sigils.empty:
                        idx_min_sigils = df_sigils.groupby('tier')['prix_sigil'].idxmin()
                        best_sigils = df_sigils.loc[idx_min_sigils]
                    else:
                        best_sigils = pd.DataFrame()
                        
                    if not df_bases.empty:
                        idx_min_bases = df_bases.groupby(['item_id', 'quality'])['prix_base'].idxmin()
                        best_bases = df_bases.loc[idx_min_bases]
                    else:
                        best_bases = pd.DataFrame()
                    ...
                    if not best_bases.empty and not best_sigils.empty:
                        for _, base_row in best_bases.iterrows():
                            ...
                            royal_target_id = base_id.replace('SET1', 'ROYAL')
                            
                            sigil_row = best_sigils[best_sigils['tier'] == tier]
                            prix_vente_row = df_royals[(df_royals['tier'] == tier) & (df_royals['item_id'] == royal_target_id) & (df_royals['quality'] == qualite)]
                            
                            if not sigil_row.empty and not prix_vente_row.empty:
                                prix_sigil = sigil_row.iloc[0]['prix_sigil']
                                ville_sigil = sigil_row.iloc[0]['city']
                                prix_vente_bm = prix_vente_row.iloc[0]['prix_vente_bm']
                                ...
                                lignes_forge.append({
                                    "Piece": traduire_nom_piece(royal_target_id),
                    ...
                    if not lignes_forge:
                        ...
                        df_final_forge = pd.DataFrame(lignes_forge).sort_values("Profit Net", ascending=False).head(25).reset_index(drop=True)
                        df_final_forge.index += 1
                        
                        st.success("Affichage du Top 25 des operations de forge (rentables ou non).")
                        
                        df_style_forge = df_final_forge.copy()
                        df_style_forge['Prix Base'] = df_style_forge['Prix Base'].map('{:,.0f}'.format)
                        df_style_forge['Prix 1 Sigil'] = df_style_forge['Prix 1 Sigil'].map('{:,.0f}'.format)
                        df_style_forge['Cout Fabrication'] = df_style_forge['Cout Fabrication'].map('{:,.0f}'.format)
                        df_style_forge['Revente MN'] = df_style_forge['Revente MN'].map('{:,.0f}'.format)
                        df_style_forge['Profit Net'] = df_style_forge['Profit Net'].map('{:,.0f}'.format)
                        
                        st.dataframe(df_style_forge, use_container_width=True, height=500)
            except Exception as e:
                st.error(f"Erreur : {e}")